...
Two-stage training for PoreSR and PoreSR-GAN:
    Stage 1: Reconstruction (L1 + MS-SSIM + Gradient loss)
    Stage 2: Adversarial fine-tuning (PatchGAN, optional)

Supports all four trained models in the ablation study:
    - SRResNet-2D:   in_channels=1, use_cbam=False
    - SRGAN-2D:      in_channels=1, use_cbam=False, + Stage 2
...
def train_stage1(model, train_loader, val_loader, config, save_dir, model_name):
    ...
    Uses Adam optimiser with linear warmup and cosine annealing.
    Mixed-precision training and gradient clipping are applied throughout.
    """
...
def train_stage2(generator, train_loader, val_loader, config, save_dir,
                 model_name):
    """
    Stage 2: Adversarial fine-tuning with PatchGAN discriminator.

    Uses hinge loss for the discriminator and a conservative adversarial
    weight (lambda_adv = 0.001) to limit structural disruption.
    """
    device = torch.device("cuda")
    ...
    # Save final GAN generator
    gan_path = os.path.join(save_dir, f"generator_gan_step_{config['gan_steps']}.pth")
    torch.save({
...
def main():
    parser = argparse.ArgumentParser(description="PoreSR Training")
    ...
    if args.model in ("SRResNet_2D", "SRGAN_2D"):
        ...
        use_cbam = False
        k = config["k_slices_2d"]
    else:
        in_channels = config["k_slices_25d"]
        use_cbam = True
        k = config["k_slices_25d"]
    ...
    # Stage 2: GAN (if applicable)
    if args.model in ("SRGAN_2D", "PoreSR_GAN"):
        model = train_stage2(
            model, train_loader, val_loader, config, args.output_dir, args.model
        )
